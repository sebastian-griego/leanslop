...
theorem tested_hahn_banach (f : E →ₗ.[ℝ] ℝ) (N : E → ℝ)
  ...
  let F : (E × ℝ) →ₗ.[ℝ] ℝ :=
    { domain := D
      toFun := second - f.toFun.comp first }
  ...
  let S : PointedCone ℝ (E × ℝ) :=
    { carrier := {p | N p.1 ≤ p.2}
      ...
      smul_mem' := by
        intro c p hp
        change N ((c : ℝ) • p.1) ≤ (c : ℝ) * p.2
        rcases c.2.eq_or_lt with hc | hc
        · have hc' : (c : ℝ) = 0 := hc.symm
          simp [hc', hN0]
        · rw [N_hom (c : ℝ) hc p.1]
          exact mul_le_mul_of_nonneg_left hp c.2 }
  ...
  refine ⟨g, ?_, ?_⟩
  · intro x
    have h := hG (⟨((x : E), (0 : ℝ)), x.2⟩ : F.domain)
