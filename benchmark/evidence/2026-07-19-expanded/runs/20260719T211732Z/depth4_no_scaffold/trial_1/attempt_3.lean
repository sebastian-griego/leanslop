...
theorem tested_hahn_banach (f : E →ₗ.[ℝ] ℝ) (N : E → ℝ)
    ...
    { domain := f.domain
      toMap :=
        { toFun := fun x => f x
          ...
          map_smul' := fun c x => f.map_smul c x }
      base_le := fun _ hx => hx
      base_eq := fun _ => rfl
      bound := hf }
  obtain ⟨m, hm⟩ := zorn_le (α := HBExtension f N) (by
    ...
    · let D : Submodule ℝ E :=
      ...
      have v_eq (x : D) (a : HBExtension f N) (ha : a ∈ c)
          (hxa : (x : E) ∈ a.domain) :
          v x = a.toMap ⟨x, hxa⟩ :=
        common_value (pick_mem x) ha (pick_dom x) hxa
      let L : D →ₗ[ℝ] ℝ :=
        { toFun := v
          map_add' := by
            intro x y
            ...
            · have hy : (y : E) ∈ (pick x).domain := hyx.1 (pick_dom y)
              have hxy' : ((x + y : D) : E) ∈ (pick x).domain :=
                (pick x).domain.add_mem (pick_dom x) hy
              calc
                v (x + y) =
                    (pick x).toMap ⟨(x + y : D), hxy'⟩ :=
                  v_eq (x + y) (pick x) (pick_mem x) hxy'
                _ = (pick x).toMap ⟨x, pick_dom x⟩ +
                    (pick x).toMap ⟨y, hy⟩ := by
                  simpa using
                    (pick x).toMap.map_add
                      (⟨x, pick_dom x⟩ : (pick x).domain)
                      (⟨y, hy⟩ : (pick x).domain)
                _ = v x + v y := by
                  rw [v_eq x (pick x) (pick_mem x) (pick_dom x),
                    v_eq y (pick x) (pick_mem x) hy]
          map_smul' := by
            intro r x
      ...
      · intro x hx
        exact ⟨a, ha, hx⟩
      · intro x
        exact v_eq ⟨x, ⟨a, ha, x.property⟩⟩ a ha x.property
    · exact ⟨initial, by
        intro a ha
        exact (hcn ⟨a, ha⟩).elim⟩)
  have mtop : m.domain = ⊤ := by
    ...
    have hc_lower (y : m.domain) :
      ...
      have hmap :
          m.toMap (y + z) = m.toMap y + m.toMap z :=
        m.toMap.map_add y z
      have hn := N_add ((y : E) - x) ((z : E) + x)
      have heq :
          ((y : E) - x) + ((z : E) + x) = (y + z : m.domain) := by
        change ((y : E) - x) + ((z : E) + x) = (y : E) + (z : E)
        abel
      rw [heq] at hn
      rw [hmap] at hmadd
      linarith
    have general_bound (y : m.domain) (a : ℝ) :
      ...
      · let b : ℝ := -a
        ...
        have hvec :
          ...
          calc
            b • ((z : E) - x) =
                b • (z : E) - b • x := by rw [smul_sub]
            _ = (y : E) - b • x := by
              have h := congrArg Subtype.val hby
              rw [h]
            _ = (y : E) + a • x := by
              dsimp [b]
              rw [neg_smul]
      ...
      · let z : m.domain := a⁻¹ • y
        ...
        have hvec :
          ...
          calc
            a • ((z : E) + x) =
                a • (z : E) + a • x := by rw [smul_add]
            _ = (y : E) + a • x := by
              have h := congrArg Subtype.val hay
              rw [h]
        calc
          m.toMap y + a * c₀ =
              a * (m.toMap z + c₀) := by
    ...
    have coeff_unique {y y' : m.domain} {a a' : ℝ}
      ...
      have hxmem : x ∈ m.domain := by
        have heq : x = (a - a')⁻¹ • ((y' : E) - y) := by
          ...
          simp [hd]
        rw [heq]
        exact m.domain.smul_mem _
          (m.domain.sub_mem y'.property y.property)
      exact hxm hxmem
    let py (z : D) : m.domain := Classical.choose z.property
    let pa (z : D) : ℝ :=
      Classical.choose (Classical.choose_spec z.property)
    have pspec (z : D) :
        (z : E) = (py z : E) + pa z • x :=
    ...
    let u : HBExtension f N :=
      { domain := D
        ...
        base_le := by
          intro z hz
          exact ⟨⟨z, m.base_le hz⟩, (0 : ℝ), by simp⟩
        base_eq := by
          intro z
          have hz :
              ((⟨z, ⟨⟨z, m.base_le z.property⟩,
                (0 : ℝ), by simp⟩⟩ : D) : E) =
                ((⟨z, m.base_le z.property⟩ : m.domain) : E) +
                  (0 : ℝ) • x := by
            simp
          calc
            L ⟨z, ⟨⟨z, m.base_le z.property⟩,
                (0 : ℝ), by simp⟩⟩ =
                m.toMap ⟨z, m.base_le z.property⟩ := by
              simpa using
                q_eq
                  (⟨z, ⟨⟨z, m.base_le z.property⟩,
                    (0 : ℝ), by simp⟩⟩ : D)
                  (⟨z, m.base_le z.property⟩ : m.domain)
                  (0 : ℝ) hz
            _ = f z := m.base_eq z
        bound := by
          intro z
        ...
        simpa using
          q_eq
            (⟨y, ⟨⟨y, y.property⟩,
              (0 : ℝ), by simp⟩⟩ : D)
            y (0 : ℝ) hy
    have hum : u = m := le_antisymm (hm hmu) hmu
  ...
  let inclusion : E →ₗ[ℝ] m.domain :=
    { toFun := fun x =>
        ⟨x, by rw [mtop]; exact Submodule.mem_top⟩
      map_add' := fun _ _ => rfl
